...
- **Generated QA outputs:** Rebuild `pipeline/` with `python3 tools/build_pipeline_outputs.py`, rebuild extraction-review queues with `python3 tools/build_extraction_review_artifacts.py`, then merge figure-candidate audits with `python3 tools/merge_figure_candidate_audits.py`; review `pipeline/PIPELINE_QA_REPORT.md`, `digitization/source_review/EXTRACTION_REVIEW_SUMMARY.md`, and `digitization/source_review/FIGURE_CANDIDATE_AUDIT_SUMMARY.md` before using manuscript counts or extracted values.
